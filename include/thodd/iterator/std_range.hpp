#ifndef __thodd_cache_iterator_std_range_hpp__
#  define __thodd_cache_iterator_std_range_hpp__

namespace thodd {
  /// Wrapper permettant de migrer un iterator standart vers
  /// les methodes de lazy stream
  template <typename iterator_t> 
  struct std_range_iterator {
    iterator_t it ;
    iterator_t end ;
  } ;

  /** 
   * @brief  Construit un std_range_iterator à partir d'un container standart.
   * @param  begin: iterateur de début
   * @param  end: iterateur de fin
   */
  template <typename iterator_t>
  std_range_iterator<iterator_t> 
  ...
  std_range_iterator<typename stdcontainer_t::const_iterator> 
   ...
   */
  auto in (std_range_iterator<auto> const & srit) -> decltype(auto) {
    return *srit.it ;
  }
   ...
   */
  bool has_next (std_range_iterator<auto> const & srit) {
   ...
   */
  auto next (std_range_iterator<auto> & srit) -> decltype(srit) {
